...
func (e *Engine) ApplyPlan(ctx context.Context, plan *ir.Plan, state *ir.State) (*ir.State, error) {
	...
	for _, change := range plan.Changes {
		...
		// Look up prior state from the state map (ResourceState has real outputs, not config-like Prior)
		...
		case "CREATE", "UPDATE", "REPLACE":
			...
			// Replace existing entry or append new one (avoid duplicates)
			if idx, ok := stateIndex[addr]; ok {
		...
		case "DELETE":
			// Look up the resource ID from state outputs for the provider
			...
			if err != nil {
				return nil, fmt.Errorf("delete failed for %s: %w", addr, err)
			}
